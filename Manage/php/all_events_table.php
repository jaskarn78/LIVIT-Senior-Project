<?php
	...
	if (isset($_SESSION['facebook_access_token'])) {
		// GET the input from the forms within headline.php
		...
		if (!empty($nameSet) && !empty($dateSet) && !empty($sponSet)) {
			$query = sprintf("SELECT event_id, event_name, event_sponsor, event_location, preference_id, start_date, img_path
				    FROM events WHERE event_name LIKE '%%%s%%' or start_date LIKE '%%%s%%' or event_sponsor LIKE '%%%s%%'",
					$nameSet, $dateSet,  $sponSet);
		} else if (!empty($nameSet) && !empty($dateSet)) {
			$query = sprintf("SELECT event_id, event_name, event_sponsor, event_location, preference_id, start_date, img_path
					FROM events WHERE event_name LIKE '%%%s%%' or start_date LIKE '%%%s%%'", 
					$nameSet, $dateSet);
		} else if (!empty($nameSet) && !empty($sponSet)) {
			$query = sprintf("SELECT event_id, event_name, event_sponsor, event_location, preference_id, start_date, img_path
					FROM events WHERE event_name LIKE '%%%s%%' or event_sponsor LIKE '%%%s%%'", 
					$nameSet, $sponSet);
		} else if (!empty($dateSet) && !empty($sponSet)) {
			$query = sprintf("SELECT event_id, event_name, event_sponsor, event_location, preference_id, start_date, img_path
					FROM events WHERE start_date LIKE '%%%s%%' or event_sponsor LIKE '%%%s%%'", 
					$dateSet, $sponSet);
		} else if (!empty($nameSet)) {
			$query = sprintf("SELECT event_id, event_name, event_sponsor, event_location, preference_id, start_date, img_path
					FROM events WHERE event_name LIKE '%%%s%%'", $nameSet);
		} else if (!empty($sponSet)) {
			$query = sprintf("SELECT event_id, event_name, event_sponsor, event_location, preference_id, start_date, img_path
					FROM events WHERE event_sponsor LIKE '%%%s%%'", $sponSet);
		} else if (!empty($dateSet)) {
			$query = sprintf("SELECT event_id, event_name, event_sponsor, event_location, preference_id, start_date, img_path
					FROM events WHERE start_date LIKE '%%%s%%'", $dateSet);
		}
	...
	if (isset($array)) { 
		...
		for ($i = 0; $i < count($array); $i++) {
			...
			$output .= "<td onclick='document.getElementById(\"" . $array[$i]['event_id'] . "\").submit();'><img src='" . $img . "' width='90' height='90' align='middle' padding='2cm' /></td>\n";
